...
function Cart() {
  const { buynow_cart_items, cart_total_info } = useContext(UserContext);
  ...
  const handleProceedToCheckout = () => {
    ...
    handleRedirectInternal(
      history,
      "checkout",
      `?productId=${productIds.join(",")}&amount=${
        getCartTotalBeforeTax() + getCartTotalTaxes()
      }&singleProduct=0&shippingId=`
    );
  };

  useEffect(() => {
    if (isAuthenticated) {
      if (cart_total_info && Object.keys(cart_total_info).length > 0) {
        setCartTotal(cart_total_info);
      } else {
        setCartTotal(null);
      }
    }
