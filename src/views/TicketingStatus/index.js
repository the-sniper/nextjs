...
function Ticketing(props) {
  ...
  const add_total_seats = (seat) => {
    setTotalSeats(parseInt(total_ticket_price) + parseInt(seat));
    return false;
  };
    ...
    if (ticket_response) {
      if (ticket_response.from == "Ticket_System") {
        if (ticket_response.message) {
          ...
          if (ticket_response.message.stagelist.length > 0) {
            setTotalSeats(ticket_response.message.stagelist[0].total_seat);
          }
        }
